...
// New DOM elements for customer list
const customerNameInput = document.getElementById("customerName");
const customerList = document.getElementById("customerList");
const phoneNumberInput = document.getElementById("phoneNumber");

// Initialize orders and customers arrays from localStorage or start empty
let orders = JSON.parse(localStorage.getItem("orders")) || [];
let customers = JSON.parse(localStorage.getItem("customers")) || [];

// Function to populate the datalist with customer names
...
// Save customer information if not already saved
...
// Prefill the form when a customer is selected from the list
customerNameInput.addEventListener("input", function() {
  const selectedCustomer = customers.find(c => c.name === this.value);
  if (selectedCustomer) {
    phoneNumberInput.value = selectedCustomer.phone;
    // Prefill other form fields as necessary
  } else {
    phoneNumberInput.value = ""; // Clear the phone number field if no match is found
  }
});
...
function handleSubmit(event) {
  ...
  saveCustomer({ name: newOrder.customerName, phone: newOrder.phoneNumber });
}
